...
def append(model, models, weights=None):
    '''
    Append several smaller models to the larger model,
    significantly faster than combine in this case
    '''
    if weights is None:
        weights = [1. for _ in range(len(models))]
    ...
    model_dict, models_reversed_dict = get_model_dict(model)

    model_dicts = list(map(get_model_dict, models))
    # thing.model, thing.model_reversed' and thing.model 2, thing.model_reversed 2

    models_list = [md[0] for md in model_dicts]
    models_reversed_list = [md[1] for md in model_dicts]
    ...
    c_reversed = models_reversed_dict
    for m, w in zip(models_reversed_list, weights):
        for state, options in m.items():
            current = c_reversed.get(state, {})
            for subseq_k, subseq_v in options.items():
    ...
    if isinstance(ret_inst, Text):
        if model.retain_original and any(m.retain_original for m in models):
            combined_sentences = []
            for m in models:
                if m.retain_original:
                    combined_sentences += m.parsed_sentences
